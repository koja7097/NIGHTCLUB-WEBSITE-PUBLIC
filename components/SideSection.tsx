...
import { Calendar, Newspaper, ListCheck } from "lucide-react";
...
export default function SideSection({activeTap, setActiveTap}: SideSectionProps) {
    const tabs = [
        ...
        {id: "feed", label: "Feed", icon: <Newspaper size={20}/>},
        {id: "booking", label: "Booking", icon: <ListCheck size={20}/>},
    ]
    return (
        <aside className="w-64 flex-col bg-white-900 text-dark p-6 space-y-4">
            <h2 className="text-xl font-bold mb-6 ">Dashbaord Menu</h2>
            <nav className="space-y-4 flex flex-col  h-full hover:bg-blue-600 ">
                {tabs.map((tab)=> (
                    <button
                    key={tab.id}
                    onClick={()=> setActiveTap(tab.id)}
                    className={`flex md:flex-row flex-col item-center md:item-start md:sppace-x-2 p-1   hover:bg-blue-600  ${activeTap} `}
                    >
                   {/* icon visibility */}  
